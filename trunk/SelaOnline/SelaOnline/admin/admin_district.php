...
if ($_pgR["act"] == model_District::ACT_ADD)
{
	...
	if (global_common::isCLogin())
	{
		...
		$districID = $_pgR['DistricID'];
		$districID = global_editor::rteSafe(html_entity_decode($districID,ENT_COMPAT ,'UTF-8' ));
		$districtName = $_pgR['DistrictName'];
		$districtName = global_editor::rteSafe(html_entity_decode($districtName,ENT_COMPAT ,'UTF-8' ));
		$cityID = $_pgR['CityID'];
		...
		$resultID = $objDistrict->insert($districID,$districtName,$cityID,$status);
		if ($resultID)
		{
...
elseif($_pgR['act'] == model_District::ACT_UPDATE)
{
	if (global_common::isCLogin())
	{
		//l?y th?ng tin user
		...
		$resultID = $objDistrict->update($districID,$districtName,$cityID,$status);
...
elseif($_pgR['act'] == model_District::ACT_CHANGE_PAGE)
{
	$intPage = $_pgR['p'];
	
	$outPutHTML =  $objDistrict->getListDistrict($intPage);
	echo global_common::convertToXML($strMessageHeader, array('rs','inf'), array(1,$outPutHTML),array(0,1));
	return ;
}
elseif($_pgR['act'] == model_District::ACT_SHOW_EDIT)
{
	
	$strDistrictID = $_pgR['id'];
	$arrDistrict =  $objDistrict->getDistrictByID($strDistrictID);
	
	echo global_common::convertToXML($strMessageHeader, array('rs','DistricID','DistrictName','CityID','Status'), array(1,'DistricID','DistrictName','CityID','Status'),array(0,1,1,1,1));
	return ;
}
elseif ($_pgR["act"] == model_District::ACT_GET)
{		
	$sectionID = $_pgR["sect"];
...
elseif($_pgR['act'] == model_District::ACT_DELETE)
{
...
?>
<script type="text/javascript" src="<?php echo $_objSystem->locateJs('sela_District.js');?>"></script>
...
<input type="hidden" id="txtID" name="txtID" value="" />
 <center>
<br><h2 align="center">Mananage District</h2>
		<div class="input-field-border input-field-content" >
				...
				<div class="div_admin_group_content_inside" style="width: 100%; top: -20px;">
				    <table id="tblPopUp" style="width: 100%;" border="0" cellpadding="2" cellspacing="0">
                        <tbody>
						<tr>
							<td width='110'><span style='cursor:default; font-family:inherit'>DistricID</span></td>
							<td width='10'><span class='forceFillForm'></span></td>
							<td width='567'><input id='txtDistricID' name='txtDistricID' value='' style='width: 49.5%;'  maxlength='60' type='text'></td>
						</tr>
						<tr>
							<td width='110'><span style='cursor:default; font-family:inherit'>DistrictName</span></td>
							<td width='10'><span class='forceFillForm'></span></td>
							<td width='567'><input id='txtDistrictName' name='txtDistrictName' value='' style='width: 49.5%;'  maxlength='150' type='text'></td>
						</tr>
						<tr>
				...
				<div class="div_admin_group_content_inside" style="margin: 4px; display: block;" align="center">		
				  <input id="btnOK" value="OK"  style="width: 50px;" onClick="_objDistrict.btnSave_OnClick()" type="button" class="btn btn-oliver"> &nbsp;&nbsp;&nbsp;
				  <input id="btnClose" value="Cancel" align="center" style="width: 65px;" onClick="_objDistrict.showAddMode()" type="button" class="btn btn-oliver">  
			  </div>					
		</div>	
...
<!--End Form Input -->
   ...
   <div  id="content-admin" >
					...
					<div id="list-content" style="padding:10px">
						<?php echo $objDistrict ->getListDistrict(1) ?>					
						</div>
	</div>
