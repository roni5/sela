...
include_once('class/model_role.php');

?>
<?php

$objRole = new Model_Role($objConnection);

if ($_pgR["act"] == model_Role::ACT_ADD)
{
	...
	if (global_common::isCLogin())
	{
		...
		$roleName = $_pgR['RoleName'];
		$roleName = global_editor::rteSafe(html_entity_decode($roleName,ENT_COMPAT ,'UTF-8' ));
		$status = $_pgR['Status'];
		...
		$resultID = $objRole->insert($roleID,$roleName,$status);
		if ($resultID)
		{
...
elseif($_pgR['act'] == model_Role::ACT_UPDATE)
{
	if (global_common::isCLogin())
	{
		//l?y th?ng tin user
		...
		$resultID = $objRole->update($roleID,$roleName,$status);
...
elseif($_pgR['act'] == model_Role::ACT_CHANGE_PAGE)
{
	$intPage = $_pgR['p'];
	
	$outPutHTML =  $objRole->getListRole($intPage);
	echo global_common::convertToXML($strMessageHeader, array('rs','inf'), array(1,$outPutHTML),array(0,1));
	return ;
}
elseif($_pgR['act'] == model_Role::ACT_SHOW_EDIT)
{
	
	$strRoleID = $_pgR['id'];
	$arrRole =  $objRole->getRoleByID($strRoleID);
	
	echo global_common::convertToXML($strMessageHeader, array('rs','RoleID','RoleName','Status'), array(1,'RoleID','RoleName','Status'),array(0,1,1,1));
	return ;
}
elseif ($_pgR["act"] == model_Role::ACT_GET)
{		
	$sectionID = $_pgR["sect"];
...
elseif($_pgR['act'] == model_Role::ACT_DELETE)
{
...
?>
<script type="text/javascript" src="<?php echo $_objSystem->locateJs('sela_Role.js');?>"></script>
...
<input type="hidden" id="txtID" name="txtID" value="" />
 <center>
<br><h2 align="center">Mananage Role</h2>
		<div class="input-field-border input-field-content" >
				...
				<div class="div_admin_group_content_inside" style="width: 100%; top: -20px;">
				    <table id="tblPopUp" style="width: 100%;" border="0" cellpadding="2" cellspacing="0">
						...
						<tr>
							<td width='110'><span style='cursor:default; font-family:inherit'>RoleName</span></td>
							<td width='10'><span class='forceFillForm'></span></td>
							<td width='567'><input id='txtRoleName' name='txtRoleName' value='' style='width: 49.5%;'  maxlength='150' type='text'></td>
						</tr>
						<tr>
				...
				<div class="div_admin_group_content_inside" style="margin: 4px; display: block;" align="center">		
				  <input id="btnOK" value="OK"  style="width: 50px;" onClick="_objRole.btnSave_OnClick()" type="button" class="btn btn-oliver"> &nbsp;&nbsp;&nbsp;
				  <input id="btnClose" value="Cancel" align="center" style="width: 65px;" onClick="_objRole.showAddMode()" type="button" class="btn btn-oliver">  
			  </div>					
		</div>	
...
<!--End Form Input -->
   ...
   <div  id="content-admin" >
					...
					<div id="list-content" style="padding:10px">
						<?php echo $objRole ->getListRole(1) ?>					
						</div>
	</div>
