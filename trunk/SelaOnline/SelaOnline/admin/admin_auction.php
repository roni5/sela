...
include_once('class/model_auction.php');

?>
<?php

$objAuction = new Model_Auction($objConnection);

if ($_pgR["act"] == model_Auction::ACT_ADD)
{
	...
	if (global_common::isCLogin())
	{
		...
		$startingPrice = $_pgR['StartingPrice'];
		$startingPrice = global_editor::rteSafe(html_entity_decode($startingPrice,ENT_COMPAT ,'UTF-8' ));
		$numOffer = $_pgR['NumOffer'];
		$numOffer = global_editor::rteSafe(html_entity_decode($numOffer,ENT_COMPAT ,'UTF-8' ));
		$status = $_pgR['Status'];
		...
		$resultID = $objAuction->insert($auctionID,$productID,$startDate,$endDate,$startingPrice,$numOffer,$status,);
		if ($resultID)
		{
...
elseif($_pgR['act'] == model_Auction::ACT_UPDATE)
{
	if (global_common::isCLogin())
	{
		//l?y th?ng tin user
		...
		$resultID = $objAuction->update($auctionID,$productID,$startDate,$endDate,$startingPrice,$numOffer,$status,);
...
elseif($_pgR['act'] == model_Auction::ACT_CHANGE_PAGE)
{
	$intPage = $_pgR['p'];
	
	$outPutHTML =  $objAuction->getListAuction($intPage);
	echo global_common::convertToXML($strMessageHeader, array('rs','inf'), array(1,$outPutHTML),array(0,1));
	return ;
}
elseif($_pgR['act'] == model_Auction::ACT_SHOW_EDIT)
{
	
	$strAuctionID = $_pgR['id'];
	$arrAuction =  $objAuction->getAuctionByID($strAuctionID);
	
	echo global_common::convertToXML($strMessageHeader, array('rs','AuctionID','ProductID','StartDate','EndDate','StartingPrice','NumOffer','Status',), array(1,'AuctionID','ProductID','StartDate','EndDate','StartingPrice','NumOffer','Status',),array(0,1,1,1,1,0,0,1,));
	return ;
}
elseif ($_pgR["act"] == model_Auction::ACT_GET)
{		
	$sectionID = $_pgR["sect"];
...
elseif($_pgR['act'] == model_Auction::ACT_DELETE)
{
...
?>
<script type="text/javascript" src="<?php echo $_objSystem->locateJs('sela_Auction.js');?>"></script>
...
<input type="hidden" id="txtID" name="txtID" value="" />
 <center>
<br><h2 align="center">Mananage Auction</h2>
		<div class="input-field-border input-field-content" >
				...
				<div class="div_admin_group_content_inside" style="width: 100%; top: -20px;">
				    <table id="tblPopUp" style="width: 100%;" border="0" cellpadding="2" cellspacing="0">
						...
						<tr>
							<td width='110'><span style='cursor:default; font-family:inherit'>StartingPrice</span></td>
							<td width='10'><span class='forceFillForm'></span></td>
						</tr>
						<tr>
							<td width='110'><span style='cursor:default; font-family:inherit'>NumOffer</span></td>
							<td width='10'><span class='forceFillForm'></span></td>
							<td width='567'><input id='txtNumOffer' name='txtNumOffer' value='' style='width: 49.5%;'  maxlength='20' type='text'></td>
						</tr>
						<tr>
				...
				<div class="div_admin_group_content_inside" style="margin: 4px; display: block;" align="center">		
				  <input id="btnOK" value="OK"  style="width: 50px;" onClick="_objAuction.btnSave_OnClick()" type="button" class="btn btn-oliver"> &nbsp;&nbsp;&nbsp;
				  <input id="btnClose" value="Cancel" align="center" style="width: 65px;" onClick="_objAuction.showAddMode()" type="button" class="btn btn-oliver">  
			  </div>					
		</div>	
...
<!--End Form Input -->
   ...
   <div  id="content-admin" >
					...
					<div id="list-content" style="padding:10px">
						<?php echo $objAuction ->getListAuction(1) ?>					
						</div>
	</div>
