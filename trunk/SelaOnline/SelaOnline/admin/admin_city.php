...
if ($_pgR["act"] == model_City::ACT_ADD)
{
	...
	if (global_common::isCLogin())
	{
		...
		$cityName = $_pgR['CityName'];
		$cityName = global_editor::rteSafe(html_entity_decode($cityName,ENT_COMPAT ,'UTF-8' ));
		$status = $_pgR['Status'];
		...
		$resultID = $objCity->insert($cityid,$cityname,$status);
		if ($resultID)
		{
...
elseif($_pgR['act'] == model_City::ACT_UPDATE)
{
	if (global_common::isCLogin())
	{
		//l?y th?ng tin user
		...
		$resultID = $objCity->update($cityid,$cityname,$status);
...
elseif($_pgR['act'] == model_City::ACT_CHANGE_PAGE)
{
	$intPage = $_pgR['p'];
	
	$outPutHTML =  $objCity->getListCity($intPage);
	echo global_common::convertToXML($strMessageHeader, array('rs','inf'), array(1,$outPutHTML),array(0,1));
	return ;
}
elseif($_pgR['act'] == model_City::ACT_SHOW_EDIT)
{
	
	$strCityID = $_pgR['id'];
	$arrCity =  $objCity->getCityByID($strCityID);
	
	echo global_common::convertToXML($strMessageHeader, array('rs','CityID','CityName','Status'), array(1,'CityID','CityName','Status'),array(0,1,1,1));
	return ;
}
elseif ($_pgR["act"] == model_City::ACT_GET)
{		
	$sectionID = $_pgR["sect"];
...
elseif($_pgR['act'] == model_City::ACT_DELETE)
{
...
?>
<script type="text/javascript" src="<?php echo $_objSystem->locateJs('sela_City.js');?>"></script>
...
<input type="hidden" id="txtID" name="txtID" value="" />
 <center>
<br><h2 align="center">Mananage City</h2>
		<div class="input-field-border input-field-content" >
				...
				<div class="div_admin_group_content_inside" style="width: 100%; top: -20px;">
				    <table id="tblPopUp" style="width: 100%;" border="0" cellpadding="2" cellspacing="0">
						...
						<tr>
							<td width='110'><span style='cursor:default; font-family:inherit'>CityName</span></td>
							<td width='10'><span class='forceFillForm'></span></td>
							<td width='567'><input id='txtCityName' name='txtCityName' value='' style='width: 49.5%;'  maxlength='150' type='text'></td>
						</tr>
						<tr>
				...
				<div class="div_admin_group_content_inside" style="margin: 4px; display: block;" align="center">		
				  <input id="btnOK" value="OK"  style="width: 50px;" onClick="_objCity.btnSave_OnClick()" type="button" class="btn btn-oliver"> &nbsp;&nbsp;&nbsp;
				  <input id="btnClose" value="Cancel" align="center" style="width: 65px;" onClick="_objCity.showAddMode()" type="button" class="btn btn-oliver">  
			  </div>					
		</div>	
...
<!--End Form Input -->
   ...
   <div  id="content-admin" >
					...
					<div id="list-content" style="padding:10px">
						<?php echo $objCity ->getListCity(1) ?>					
						</div>
	</div>
