 ...
 * Date Created 5/6/2012 1:15:47 PM
 *
 */
...
{		   
	...
	*
	*/
	public function  Model_City($objConnection)
	{
    ...
    public function insert( $cityname,$createdby,$createddate,$modifiedby,$modifieddate,$deletedby,$deleteddate,$isdeleted,$status)
	{
		...
		$strSQL = global_common::prepareQuery(self::SQL_INSERT_SL_CITY,
				array(self::TBL_SL_CITY,$intID,global_common::escape_mysql_string($cityname),global_common::escape_mysql_string($createdby),global_common::escape_mysql_string($createddate),global_common::escape_mysql_string($modifiedby),global_common::escape_mysql_string($modifieddate),global_common::escape_mysql_string($deletedby),global_common::escape_mysql_string($deleteddate),global_common::escape_mysql_string($isdeleted),global_common::escape_mysql_string($status)));
		
		if (!global_common::ExecutequeryWithCheckExistedTable($strSQL,self::SQL_CREATE_TABLE_SL_CITY,$this->_objConnection,$strTableName))
		{
			//echo $strSQL;
			global_common::writeLog('Error add sl_city:'.$strSQL,1);
			return false;
		}	
		return $intID;
	...
	{		
		...
		if(!$arrResult)
		{
			global_common::writeLog('get sl_city ByID:'.$strSQL,1,$_mainFrame->pPage);
			return null;
		}
		//print_r($arrResult);
		return $arrResult[0];
    ...
    public function getAllCity($selectField='*') 
	{		
		$strSQL .= global_common::prepareQuery(global_common::SQL_SELECT_FREE, 
				array($selectField, self::TBL_SL_CITY ,							
					''));
		...
		if(!$arrResult)
		{
			global_common::writeLog('get All sl_city:'.$strSQL,1,$_mainFrame->pPage);
			return null;
		}
		//print_r($arrResult);
		return $arrResult;
	...
	{		
		...
		$strHTML .= "<div>".global_common::getPagingHTMLByNum($intPage,self::NUM_PER_PAGE,global_common::getTotalRecord(self::TBL_SL_CITY,$this->_objConnection),
