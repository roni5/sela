...
{		   
    ...
    var ACT_GET = 15;
    var _strPage = "admin_setting.php";
    ...
    function btnSave_OnClick() {
		...
		var settingID = core.trim(core.getObject("txtSettingID").val());
		core.ValidateInputTextBox('txtSettingID','');
		if(settingID == ''){
			core.ValidateInputTextBox('txtSettingID','SettingID is required', isValid);
			isValid =  false;
		}else if (settingID.length > 20) {
			core.ValidateInputTextBox('txtSettingID','SettingID must be less than 20', isValid);
			isValid =  false;
		}

		var settingName = core.trim(core.getObject("txtSettingName").val());
		core.ValidateInputTextBox('txtSettingName','');
		if(settingName == ''){
			core.ValidateInputTextBox('txtSettingName','SettingName is required', isValid);
			isValid =  false;
		}else if (settingName.length > 50) {
			core.ValidateInputTextBox('txtSettingName','SettingName must be less than 50', isValid);
			isValid =  false;
		}

		var settingValue = core.trim(core.getObject("txtSettingValue").val());
		core.ValidateInputTextBox('txtSettingValue','');
		if(settingValue == ''){
			core.ValidateInputTextBox('txtSettingValue','SettingValue is required', isValid);
			isValid =  false;
		}else if (settingValue.length > 255) {
			core.ValidateInputTextBox('txtSettingValue','SettingValue must be less than 255', isValid);
			isValid =  false;
		}
    ...
    function edit() {
        ...
        strRequest = "?isAJ=1&act=" + ACT_UPDATE +  
            '&SettingID='+ core.urlencode(settingID)+
			'&SettingName='+ core.urlencode(settingName)+
			'&SettingValue='+ core.urlencode(settingValue)+
    ...
    function deleteObj(id, name) {
        //curRow = currentRowId;
        popDiv.alert('Do you want to delete ' + name + ' ?', SYSTEM_TITLE_ERROR, 2, "_objSetting.delete_OK()", "_objSetting.delete_Cancel()");
        ...
        if (xmlHTTPRequest.readyState == 4) {
            if (xmlHTTPRequest.status == 200) {
                ...
                if (parseInt(strRespond[1]['rs']) == 1) {
                    ...
                    // Add Doc && clear field
					core.getObject('txtSettingID').val(SettingID);
					core.getObject('txtSettingName').val(SettingName);
					core.getObject('txtSettingValue').val(SettingValue);
    ...
    function showAddMode() {
		...
		core.getObject('txtSettingID').val('');
		core.getObject('txtSettingName').val('');
		core.getObject('txtSettingValue').val('');
		core.getObject('txtStatus').val('');
    }
    //endregion   
}
var _objSetting = new  Setting();
