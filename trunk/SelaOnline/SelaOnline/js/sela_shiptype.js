...
{		   
    ...
    var ACT_GET = 15;
    var _strPage = "admin_shiptype.php";
    ...
    function btnSave_OnClick() {
		...
		var shipTypeID = core.trim(core.getObject("txtShipTypeID").val());
		core.ValidateInputTextBox('txtShipTypeID','');
		if(shipTypeID == ''){
			core.ValidateInputTextBox('txtShipTypeID','ShipTypeID is required', isValid);
			isValid =  false;
		}else if (shipTypeID.length > 20) {
			core.ValidateInputTextBox('txtShipTypeID','ShipTypeID must be less than 20', isValid);
			isValid =  false;
		}

		var shipTypeName = core.trim(core.getObject("txtShipTypeName").val());
		core.ValidateInputTextBox('txtShipTypeName','');
		if(shipTypeName == ''){
			core.ValidateInputTextBox('txtShipTypeName','ShipTypeName is required', isValid);
			isValid =  false;
		}else if (shipTypeName.length > 50) {
			core.ValidateInputTextBox('txtShipTypeName','ShipTypeName must be less than 50', isValid);
			isValid =  false;
		}
    ...
    function edit() {
        ...
        strRequest = "?isAJ=1&act=" + ACT_UPDATE +  
            '&ShipTypeID='+ core.urlencode(shipTypeID)+
			'&ShipTypeName='+ core.urlencode(shipTypeName)+		;
    ...
    function deleteObj(id, name) {
        //curRow = currentRowId;
        popDiv.alert('Do you want to delete ' + name + ' ?', SYSTEM_TITLE_ERROR, 2, "_objShipType.delete_OK()", "_objShipType.delete_Cancel()");
        ...
        if (xmlHTTPRequest.readyState == 4) {
            if (xmlHTTPRequest.status == 200) {
                ...
                if (parseInt(strRespond[1]['rs']) == 1) {
                    ...
                    // Add Doc && clear field
					core.getObject('txtShipTypeID').val(ShipTypeID);
					core.getObject('txtShipTypeName').val(ShipTypeName);
    ...
    function showAddMode() {
		...
		core.getObject('txtShipTypeID').val('');
		core.getObject('txtShipTypeName').val('');
    }
    //endregion   
}
var _objShipType = new  ShipType();
