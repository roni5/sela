...
include_once('class/model_numberaire.php');

?>
<?php

$objNumberaire = new Model_Numberaire($objConnection);

if ($_pgR["act"] == model_Numberaire::ACT_ADD)
{
	...
	if (global_common::isCLogin())
	{
		...
		$numberaireName = $_pgR['NumberaireName'];
		$numberaireName = global_editor::rteSafe(html_entity_decode($numberaireName,ENT_COMPAT ,'UTF-8' ));
		$status = $_pgR['Status'];
		...
		$resultID = $objNumberaire->insert($numberaireID,$numberaireName,$status);
		if ($resultID)
		{
...
elseif($_pgR['act'] == model_Numberaire::ACT_UPDATE)
{
	if (global_common::isCLogin())
	{
		//l?y th?ng tin user
		...
		$resultID = $objNumberaire->update($numberaireID,$numberaireName,$status);
...
elseif($_pgR['act'] == model_Numberaire::ACT_CHANGE_PAGE)
{
	$intPage = $_pgR['p'];
	
	$outPutHTML =  $objNumberaire->getListNumberaire($intPage);
	echo global_common::convertToXML($strMessageHeader, array('rs','inf'), array(1,$outPutHTML),array(0,1));
	return ;
}
elseif($_pgR['act'] == model_Numberaire::ACT_SHOW_EDIT)
{
	
	$strNumberaireID = $_pgR['id'];
	$arrNumberaire =  $objNumberaire->getNumberaireByID($strNumberaireID);
	
	echo global_common::convertToXML($strMessageHeader, array('rs','NumberaireID','NumberaireName','Status'), array(1,'NumberaireID','NumberaireName','Status'),array(0,1,1,1));
	return ;
}
elseif ($_pgR["act"] == model_Numberaire::ACT_GET)
{		
	$sectionID = $_pgR["sect"];
...
elseif($_pgR['act'] == model_Numberaire::ACT_DELETE)
{
...
?>
<script type="text/javascript" src="<?php echo $_objSystem->locateJs('sela_Numberaire.js');?>"></script>
...
<input type="hidden" id="txtID" name="txtID" value="" />
 <center>
<br><h2 align="center">Mananage Numberaire</h2>
		<div class="input-field-border input-field-content" >
				...
				<div class="div_admin_group_content_inside" style="width: 100%; top: -20px;">
				    <table id="tblPopUp" style="width: 100%;" border="0" cellpadding="2" cellspacing="0">
						...
						<tr>
							<td width='110'><span style='cursor:default; font-family:inherit'>NumberaireName</span></td>
							<td width='10'><span class='forceFillForm'></span></td>
							<td width='567'><input id='txtNumberaireName' name='txtNumberaireName' value='' style='width: 49.5%;'  maxlength='150' type='text'></td>
						</tr>
						<tr>
				...
				<div class="div_admin_group_content_inside" style="margin: 4px; display: block;" align="center">		
				  <input id="btnOK" value="OK"  style="width: 50px;" onClick="_objNumberaire.btnSave_OnClick()" type="button" class="btn btn-oliver"> &nbsp;&nbsp;&nbsp;
				  <input id="btnClose" value="Cancel" align="center" style="width: 65px;" onClick="_objNumberaire.showAddMode()" type="button" class="btn btn-oliver">  
			  </div>					
		</div>	
...
<!--End Form Input -->
   ...
   <div  id="content-admin" >
					...
					<div id="list-content" style="padding:10px">
						<?php echo $objNumberaire ->getListNumberaire(1) ?>					
						</div>
	</div>
