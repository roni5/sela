...
$objNotificationType = new Model_NotificationType($objConnection);

if ($_pgR["act"] == model_NotificationType::ACT_ADD)
{
	...
	if (global_common::isCLogin())
	{
		...
		$notificationTypeID = $_pgR['NotificationTypeID'];
		$notificationTypeID = global_editor::rteSafe(html_entity_decode($notificationTypeID,ENT_COMPAT ,'UTF-8' ));
		$notificationTypeName = $_pgR['NotificationTypeName'];
		$notificationTypeName = global_editor::rteSafe(html_entity_decode($notificationTypeName,ENT_COMPAT ,'UTF-8' ));
		$status = $_pgR['Status'];
		...
		$resultID = $objNotificationType->insert($notificationTypeID,$notificationTypeName,$status);
		if ($resultID)
		{
...
elseif($_pgR['act'] == model_NotificationType::ACT_UPDATE)
{
	if (global_common::isCLogin())
	{
		//l?y th?ng tin user
		...
		$resultID = $objNotificationType->update($notificationTypeID,$notificationTypeName,$status);
...
elseif($_pgR['act'] == model_NotificationType::ACT_CHANGE_PAGE)
{
	$intPage = $_pgR['p'];
	
	$outPutHTML =  $objNotificationType->getListNotificationType($intPage);
	echo global_common::convertToXML($strMessageHeader, array('rs','inf'), array(1,$outPutHTML),array(0,1));
	return ;
}
elseif($_pgR['act'] == model_NotificationType::ACT_SHOW_EDIT)
{
	
	$strNotificationTypeID = $_pgR['id'];
	$arrNotificationType =  $objNotificationType->getNotificationTypeByID($strNotificationTypeID);
	
	echo global_common::convertToXML($strMessageHeader, array('rs','NotificationTypeID','NotificationTypeName','Status'), array(1,'NotificationTypeID','NotificationTypeName','Status'),array(0,1,1,1));
	return ;
}
elseif ($_pgR["act"] == model_NotificationType::ACT_GET)
{		
	$sectionID = $_pgR["sect"];
...
elseif($_pgR['act'] == model_NotificationType::ACT_DELETE)
{
...
?>
<script type="text/javascript" src="<?php echo $_objSystem->locateJs('sela_NotificationType.js');?>"></script>
...
<input type="hidden" id="txtID" name="txtID" value="" />
 <center>
<br><h2 align="center">Mananage NotificationType</h2>
		<div class="input-field-border input-field-content" >
				...
				<div class="div_admin_group_content_inside" style="width: 100%; top: -20px;">
				    <table id="tblPopUp" style="width: 100%;" border="0" cellpadding="2" cellspacing="0">
                        <tbody>
						<tr>
							<td width='110'><span style='cursor:default; font-family:inherit'>NotificationTypeID</span></td>
							<td width='10'><span class='forceFillForm'></span></td>
							<td width='567'><input id='txtNotificationTypeID' name='txtNotificationTypeID' value='' style='width: 49.5%;'  maxlength='60' type='text'></td>
						</tr>
						<tr>
							<td width='110'><span style='cursor:default; font-family:inherit'>NotificationTypeName</span></td>
							<td width='10'><span class='forceFillForm'></span></td>
							<td width='567'><input id='txtNotificationTypeName' name='txtNotificationTypeName' value='' style='width: 49.5%;'  maxlength='150' type='text'></td>
						</tr>
						<tr>
				...
				<div class="div_admin_group_content_inside" style="margin: 4px; display: block;" align="center">		
				  <input id="btnOK" value="OK"  style="width: 50px;" onClick="_objNotificationType.btnSave_OnClick()" type="button" class="btn btn-oliver"> &nbsp;&nbsp;&nbsp;
				  <input id="btnClose" value="Cancel" align="center" style="width: 65px;" onClick="_objNotificationType.showAddMode()" type="button" class="btn btn-oliver">  
			  </div>					
		</div>	
...
<!--End Form Input -->
   ...
   <div  id="content-admin" >
					...
					<div id="list-content" style="padding:10px">
						<?php echo $objNotificationType ->getListNotificationType(1) ?>					
						</div>
	</div>
