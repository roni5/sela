...
{		   
    ...
    const TBL_SL_CURRENCY			            = 'sl_currency';

	const SQL_INSERT_SL_CURRENCY		= 'INSERT INTO `{0}`
		(
			CurrencyID,
			CurrencyName,
			EuroRate,
	...
	*
	*/
	public function  Model_Currency($objConnection)
	{
    ...
    public function insert( $currencyname,$eurorate,$usdrate,$audrate,$ndtrate,$cadrate,$gbprate,$createdby,$createddate,$modifiedby,$modifieddate,$deletedby,$deleteddate,$isdeleted)
	{
		...
		$strSQL = global_common::prepareQuery(self::SQL_INSERT_SL_CURRENCY,
	...
	{
		$strTableName = self::TBL_SL_CURRENCY;
		$strSQL = global_common::prepareQuery(self::SQL_UPDATE_SL_CURRENCY,
    ...
    public function getCurrencyByID($objID,$selectField='*') 
	{		
		$strSQL .= global_common::prepareQuery(global_common::SQL_SELECT_FREE, 
				array($selectField, self::TBL_SL_CURRENCY ,							
					'WHERE CurrencyID = \''.$objID.'\' '));
		//echo '<br>SQL:'.$strSQL;
		$arrResult =$this->_objConnection->selectCommand($strSQL);		
		if(!$arrResult)
		{
			global_common::writeLog('get sl_currency ByID:'.$strSQL,1,$_mainFrame->pPage);
			return null;
		}
		//print_r($arrResult);
		return $arrResult[0];
    ...
    public function getAllCurrency($selectField='*') 
	{		
		$strSQL .= global_common::prepareQuery(global_common::SQL_SELECT_FREE, 
				array($selectField, self::TBL_SL_CURRENCY ,							
					''));
		...
		if(!$arrResult)
		{
			global_common::writeLog('get All sl_currency:'.$strSQL,1,$_mainFrame->pPage);
			return null;
		}
		//print_r($arrResult);
		return $arrResult;
    ...
    public function getListCurrency($intPage,$orderBy='CurrencyID', $whereClause)
	{		
        if($whereClause)
        {
		...
		$strSQL .= global_common::prepareQuery(global_common::SQL_SELECT_FREE,array('*',
					self::TBL_SL_CURRENCY,$orderBy.' '.$whereClause.' limit '.(($intPage-1)* self::NUM_PER_PAGE).','.self::NUM_PER_PAGE));
		//echo 'sql:'.$strSQL;	
		$arrResult = $this->_objConnection->selectCommand($strSQL);
		//print_r($arrResult);
		...
		for($i=0;$i<$icount;$i++)
		{
			$strHTML.='<tr class="'.($i%2==0?'even':'odd').'">
						<td>'.$arrResult[$i]['CurrencyID'].'</td>
						<td>'.$arrResult[$i]['CurrencyName'].'</td>
						<td>'.$arrResult[$i]['EuroRate'].'</td>
						<td>'.$arrResult[$i]['USDRate'].'</td>
						<td>'.$arrResult[$i]['AUDRate'].'</td>
						<td>'.$arrResult[$i]['NDTRate'].'</td>
						<td>'.$arrResult[$i]['CADRate'].'</td>
						<td>'.$arrResult[$i]['GBPRate'].'</td>
						...
						<td class="lastCell"><input type="checkbox" onclick="_objCurrency.showHide(\''.$arrResult[$i]['CurrencyID'].'\',\''.$arrResult[$i]['name'].'\',this)" '.($arrResult[$i]['IsDeleted']?'':'checked=checked').' /></td>
					  </tr>';
		}
		$strHTML.='</tbody></table>';
		
		$strHTML .= "<div>".global_common::getPagingHTMLByNum($intPage,self::NUM_PER_PAGE,global_common::getTotalRecord(self::TBL_SL_CURRENCY,$this->_objConnection),
