...
{		   
    ...
    var ACT_GET = 15;
    var _strPage = "admin_district.php";
    ...
    function btnSave_OnClick() {
		...
		var districID = core.trim(core.getObject("txtDistricID").val());
		core.ValidateInputTextBox('txtDistricID','');
		if(districID == ''){
			core.ValidateInputTextBox('txtDistricID','DistricID is required', isValid);
			isValid =  false;
		}else if (districID.length > 20) {
			core.ValidateInputTextBox('txtDistricID','DistricID must be less than 20', isValid);
			isValid =  false;
		}

		var districtName = core.trim(core.getObject("txtDistrictName").val());
		core.ValidateInputTextBox('txtDistrictName','');
		if(districtName == ''){
			core.ValidateInputTextBox('txtDistrictName','DistrictName is required', isValid);
			isValid =  false;
		}else if (districtName.length > 50) {
			core.ValidateInputTextBox('txtDistrictName','DistrictName must be less than 50', isValid);
			isValid =  false;
		}
    ...
    function edit() {
        ...
        strRequest = "?isAJ=1&act=" + ACT_UPDATE +  
            '&DistricID='+ core.urlencode(districID)+
			'&DistrictName='+ core.urlencode(districtName)+
    ...
    function deleteObj(id, name) {
        //curRow = currentRowId;
        popDiv.alert('Do you want to delete ' + name + ' ?', SYSTEM_TITLE_ERROR, 2, "_objDistrict.delete_OK()", "_objDistrict.delete_Cancel()");
        ...
        if (xmlHTTPRequest.readyState == 4) {
            if (xmlHTTPRequest.status == 200) {
                ...
                if (parseInt(strRespond[1]['rs']) == 1) {
                    ...
                    // Add Doc && clear field
					core.getObject('txtDistricID').val(DistricID);
					core.getObject('txtDistrictName').val(DistrictName);
    ...
    function showAddMode() {
		...
		core.getObject('txtDistricID').val('');
		core.getObject('txtDistrictName').val('');
		core.getObject('txtCityID').val('');
		core.getObject('txtStatus').val('');
    }
    //endregion   
}
var _objDistrict = new  District();
