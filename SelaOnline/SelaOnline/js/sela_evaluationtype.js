...
{		   
    ...
    var ACT_GET = 15;
    var _strPage = "admin_evaluationtype.php";
    ...
    function btnSave_OnClick() {
		...
		var evaluationTypeID = core.trim(core.getObject("txtEvaluationTypeID").val());
		core.ValidateInputTextBox('txtEvaluationTypeID','');
		if(evaluationTypeID == ''){
			core.ValidateInputTextBox('txtEvaluationTypeID','EvaluationTypeID is required', isValid);
			isValid =  false;
		}else if (evaluationTypeID.length > 20) {
			core.ValidateInputTextBox('txtEvaluationTypeID','EvaluationTypeID must be less than 20', isValid);
			isValid =  false;
		}

		var evaluationTypeName = core.trim(core.getObject("txtEvaluationTypeName").val());
		core.ValidateInputTextBox('txtEvaluationTypeName','');
		if(evaluationTypeName == ''){
			core.ValidateInputTextBox('txtEvaluationTypeName','EvaluationTypeName is required', isValid);
			isValid =  false;
		}else if (evaluationTypeName.length > 50) {
			core.ValidateInputTextBox('txtEvaluationTypeName','EvaluationTypeName must be less than 50', isValid);
			isValid =  false;
		}
    ...
    function edit() {
        ...
        strRequest = "?isAJ=1&act=" + ACT_UPDATE +  
            '&EvaluationTypeID='+ core.urlencode(evaluationTypeID)+
			'&EvaluationTypeName='+ core.urlencode(evaluationTypeName)+
    ...
    function deleteObj(id, name) {
        //curRow = currentRowId;
        popDiv.alert('Do you want to delete ' + name + ' ?', SYSTEM_TITLE_ERROR, 2, "_objEvaluationType.delete_OK()", "_objEvaluationType.delete_Cancel()");
        ...
        if (xmlHTTPRequest.readyState == 4) {
            if (xmlHTTPRequest.status == 200) {
                ...
                if (parseInt(strRespond[1]['rs']) == 1) {
                    ...
                    // Add Doc && clear field
					core.getObject('txtEvaluationTypeID').val(EvaluationTypeID);
					core.getObject('txtEvaluationTypeName').val(EvaluationTypeName);
    ...
    function showAddMode() {
		...
		core.getObject('txtEvaluationTypeID').val('');
		core.getObject('txtEvaluationTypeName').val('');
		core.getObject('txtStatus').val('');
    }
    //endregion   
}
var _objEvaluationType = new  EvaluationType();
